...
export function BankAccountFormDialog({ open, account, onOpenChange, onSubmit }: Props) {
  const form = useForm<BankAccountFormValues>({
    ...
    defaultValues: { bankName: "", accountName: "", currency: "EUR" },
  });

  useEffect(() => {
    form.reset(
      account
        ? { bankName: account.bankName, accountName: account.accountName, currency: account.currency }
        : { bankName: "", accountName: "", currency: "EUR" },
    );
  }, [account, form]);
